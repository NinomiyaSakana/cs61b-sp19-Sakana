...
public class IntListTest {
     ...
     * 增加了testReverse的测试，然后在IntList中增加了reverse方法（都写在class的最后面）。
     */
    ...
    @Test(timeout = 1000) //以防止无限循环
    public void testReverse(){
        IntList A = IntList.of(1, 2, 3);
        IntList B = IntList.of(3, 2, 1);
        IntList exp=IntList.reverse(A);
        assertEquals(exp, B);

        assertNotEquals(exp, A);//判断A已经被改变了

        //处理空输入
        IntList N = IntList.of();
        IntList expN=IntList.reverse(N);
        assertEquals(N, expN);
